...
export function EducationSection() {
  ...
  return (
    <div className="space-y-6">
      <div>
        <div className="font-mono text-[clamp(11px,calc(10.4px+0.2vw),13px)] uppercase tracking-[0.25em] text-white/30 mb-1">Training Log</div>
          ...
          <div key={edu.id} id={`education-${edu.id}`} className="flex gap-4">
            ...
            <div className="flex-1 min-w-0 pb-1">
              ...
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="font-mono text-[clamp(10px,calc(9.6px+0.16vw),12px)] text-white/25 tabular-nums">{edu.period}</span>
              </div>
      ...
      <Separator />
      <div>
        <div className="font-mono text-[clamp(11px,calc(10.4px+0.2vw),13px)] uppercase tracking-[0.25em] text-white/30 mb-3">Certifications</div>
        <div className="space-y-2.5">
          {CERTIFICATIONS.map((c, i) => (
            <div key={i} className="flex items-baseline justify-between gap-2">
              <div className="flex items-center gap-2.5 min-w-0">
                <div className="w-1 h-1 rounded-full bg-white/30 shrink-0" />
                <span className="font-body text-[clamp(13px,calc(12.4px+0.24vw),15px)] text-white/55 truncate">{c.name}</span>
              </div>
              <span className="font-mono text-[clamp(10px,calc(9.6px+0.16vw),12px)] text-white/28 shrink-0">{c.issuer}</span>
            </div>
          ))}
        </div>
      </div>
